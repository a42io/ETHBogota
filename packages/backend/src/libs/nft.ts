...
import {
  MockERC721__factory as ERC721MockFactory,
  MockERC1155__factory as ERC1155MockFactory,
...
export async function getAccountAllowedNFTs(
  ...
  for (const token of allowlistNFT) {
    if (token.tokenType === "ENS") {
      ...
      // const res = await client.query({ query: gql(tokensQuery) });
      // console.log(res);
      ...
      // subdomain で lookupaddress する必要がある
    } else {
      if (token.tokenId) {
...
export function isAllowListIncluded(
  ...
  const val = allowlistNFT.find((val) => {
    if (val.tokenType === "ENS" && ensOrNFT.tokenType === "ENS") {
      ...
      const { chainId, contractAddress, tokenId } = ensOrNFT as AllowListNFT;
      const {
...
export async function isOwner(
  ...
  try {
    ...
    if (tokenType === "ERC721") {
      const contract = ERC721MockFactory.connect(contractAddress, provider);
      if (tokenId) {
      ...
      if (tokenId) {
        const contract = ERC1155MockFactory.connect(contractAddress, provider);
        // contract.uri()
