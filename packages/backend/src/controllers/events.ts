...
export const my: express.RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    const events = await getEvents(condition, orderBy, cursor as string, limit);
...
export const getENSorAddress: express.RequestHandler = async (
  req,
  res,
  next
) => {
  try {
    const event = await getEvent(req.params.eventId as string);
    if (!event) {
      return next(notFoundException(EVENT_API_ERRORS.EVENT_NOT_FOUND));
    }
    const ens = await resolveENS(event.host.address);
    const address = await lookupAddress(event.host.ens);

    return res.json({ ens, address });
  } catch (e) {
    return next(unknownException(UNKNOWN_ERROR, e as Error));
  }
};
...
export const create: express.RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    if (
      !isFuture(new Date(event.endAt as Date), new Date(event.startAt as Date))
    ) {
    ...
    if (!isValidAllowList) {
      return next(badRequestException(EVENT_API_ERRORS.INVALID_ALLOW_LIST));
    }
    ...
    const record: Omit<Event, "id" | "createdAt" | "updatedAt"> = {
      ...
      host: event.host,
      verificationNetworkOption: event.verificationNetworkOption || "scroll",
      isCanceled: false,
...
export const update: express.RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    const record: Event = {
      ...
      host: event.host,
      verificationNetworkOption: event.verificationNetworkOption,
